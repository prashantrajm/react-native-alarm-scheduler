...
type AlarmOccurrenceRelationship = 'primary' | 'deferred' | 'followUp';
...
```ts
type AlarmOccurrencePhase = 'scheduled' | 'ringing' | 'completed' | 'cancelled';
...
type AlarmOccurrenceResolution = {
...
type AlarmOccurrenceResolutionResult = {
  alarmId: string;
